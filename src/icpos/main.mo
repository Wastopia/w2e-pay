...
import W2ELedger "canister:icrc1_ledger";
...
shared (actorContext) actor class Main(_startBlock : Nat) {
  ...
  */
  ...
  private func notify() : async () {
    ...
    var response = await W2ELedger.get_transactions({
      start = start;
      length = 1;
    });
  ...
  /**
    ...
    */
  private func sendNotification(merchant : MainTypes.Merchant, transaction : W2ELedger.Transaction) : async () {
    // Managment canister
